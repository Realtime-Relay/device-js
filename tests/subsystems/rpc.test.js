...
describe("RpcManager", () => {
  ...
  describe("listen()", () => {
    it("should throw ValidationError if callback is not a function", async () => {
      await expect(rpc.listen("test", "not a function")).rejects.toThrow(
        ValidationError,
      );
    });

    it("should throw ValidationError if callback is null", async () => {
      await expect(rpc.listen("test", null)).rejects.toThrow(ValidationError);
    });

    it("should throw ValidationError if callback is undefined", async () => {
      await expect(rpc.listen("test", undefined)).rejects.toThrow(
        ValidationError,
      );
    });

    it("should throw ValidationError if callback is a number", async () => {
      await expect(rpc.listen("test", 42)).rejects.toThrow(ValidationError);
    });

    it("should throw ValidationError if callback is an object", async () => {
      await expect(rpc.listen("test", {})).rejects.toThrow(ValidationError);
    });
    ...
    it("should throw DuplicateListenerError on second listen for same name", async () => {
      await rpc.listen("reboot", () => {});
      await expect(rpc.listen("reboot", () => {})).rejects.toThrow(
        DuplicateListenerError,
      );
    });
    ...
    it("respond() should call msg.respond with encoded success envelope", async () => {
      ...
      expect(encoded).toEqual(
        JSONCodec().encode({ status: "ok", data: { result: 42 } }),
      );
    });
    ...
    it("error() should call msg.respond with encoded error envelope", async () => {
      ...
      expect(encoded).toEqual(
        JSONCodec().encode({
          status: "error",
          data: { code: "FAIL", message: "oops" },
        }),
      );
    });

    it("should throw ValidationError on invalid name characters", async () => {
      await expect(rpc.listen("bad name", () => {})).rejects.toThrow(
        ValidationError,
      );
    });

    it("should throw ValidationError on empty name", async () => {
      await expect(rpc.listen("", () => {})).rejects.toThrow(ValidationError);
    });
  });
  ...
  describe("off()", () => {
    ...
    it("should only remove the specified listener", async () => {
      ...
      const subs = transport._getSubscriptions();
      expect(
        subs["test-org.test.command.rpc.test-device.reboot"],
      ).toBeUndefined();
      expect(
        subs["test-org.test.command.rpc.test-device.update"],
      ).toBeDefined();
    });
  });
});
