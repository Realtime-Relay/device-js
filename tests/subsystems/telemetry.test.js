...
describe('TelemetryManager', () => {
    ...
    describe('publish() — no schema', () => {
        it('should publish to the correct subject', async () => {
            ...
            expect(published[0].subject).toBe('test-org.test.telemetry.test-device.temperature');
        });
        ...
        it('should throw ValidationError on invalid metric name', async () => {
            await expect(telemetry.publish('bad metric', 1))
                .rejects.toThrow();
        });

        it('should throw ValidationError on empty metric', async () => {
            await expect(telemetry.publish('', 1))
                .rejects.toThrow();
        });
    ...
    describe('publish() — with schema', () => {
        ...
        it('should throw if metric is not in schema', async () => {
            await expect(telemetry.publish('humidity', 65))
                .rejects.toThrow("metric 'humidity' is not defined in the device schema");
        });

        it('should throw for unknown metric even with valid type', async () => {
            await expect(telemetry.publish('unknown_metric', 42))
                .rejects.toThrow('not defined in the device schema');
        });

        // --- Type mismatch ---
        it('should throw if number metric gets string reading', async () => {
            await expect(telemetry.publish('temperature', 'hot'))
                .rejects.toThrow("metric 'temperature' expects type 'number', got 'string'");
        });

        it('should throw if number metric gets boolean reading', async () => {
            await expect(telemetry.publish('temperature', true))
                .rejects.toThrow("expects type 'number', got 'boolean'");
        });

        it('should throw if number metric gets object reading', async () => {
            await expect(telemetry.publish('temperature', { val: 1 }))
                .rejects.toThrow("expects type 'number', got 'json'");
        });

        it('should throw if string metric gets number reading', async () => {
            await expect(telemetry.publish('status', 42))
                .rejects.toThrow("expects type 'string', got 'number'");
        });

        it('should throw if string metric gets boolean reading', async () => {
            await expect(telemetry.publish('status', false))
                .rejects.toThrow("expects type 'string', got 'boolean'");
        });

        it('should throw if boolean metric gets number reading', async () => {
            await expect(telemetry.publish('enabled', 1))
                .rejects.toThrow("expects type 'boolean', got 'number'");
        });

        it('should throw if boolean metric gets string reading', async () => {
            await expect(telemetry.publish('enabled', 'true'))
                .rejects.toThrow("expects type 'boolean', got 'string'");
        });

        it('should throw if json metric gets number reading', async () => {
            await expect(telemetry.publish('config', 42))
                .rejects.toThrow("expects type 'json', got 'number'");
        });

        it('should throw if json metric gets string reading', async () => {
            await expect(telemetry.publish('config', 'hello'))
                .rejects.toThrow("expects type 'json', got 'string'");
        });
