import { connect, Events, DebugEvents, credsAuthenticator, JSONCodec } from "nats";
import { DeliverPolicy, jetstream, jetstreamManager, AckPolicy, ReplayPolicy } from "@nats-io/jetstream";
import { encode, decode } from "@msgpack/msgpack";
import { decode as decodeJwt } from "nats-jwt";
import { NotConnectedError, TimeoutError, ValidationError } from "./utils/errors.js";
...
export class NatsTransport {
    ...
    async subscribe(subject, callback) {
        ...
        const consumer = await this.#jetstream.consumers.get(this.#commandQueueStreamName, opts);
    ...
    async request(subject, data, opts) {
        ...
        try {
            const response = await this.#natsClient.request(subject, encoded, { timeout });
                ...
                switch (s.type) {
                    ...
                    case Events.Error:
                        if (s.data === 'NATS_PROTOCOL_ERR') {
                            this.#connected = false;
                            this._emitStatus({ type: TransportStatus.AUTH_FAILED, error: s.data });
                        }
                        break;
                }
            }
